...
_download_and_unpack_ivy_cache() {
  ...
  baseUrl="http://lang.goodrain.me/sbt/sbt-cache"
  if [ -n "$playVersion" ]; then
...
run_sbt()
{
  ...
  *)     # 2X Dyno
  ...
  status "SBT_EXTRAS_OPTS: $SBT_EXTRAS_OPTS"
  status "PROXY: $PROXY"
  # 修改sbt源
  cat > $home/repositories <<EOF
[repositories]
  ivy: http://ivy.goodrain.me/,[organization]/[module]/(scala_[scalaVersion]/)(sbt_[sbtVersion]/)[revision]/[type]s/[artifact](-[classifier]).[ext]
  maven: http://maven.goodrain.me/
EOF
  ...
  HOME="$home" sbt ${SBT_EXTRAS_OPTS} \
    ...
    -Dsbt.repository.config=$home/repositories \
