...
make_url () {
  ...
  case "$version" in
    0.7.*) echo "http://lang.goodrain.me/sbt/sbt-launch/sbt-launch-0.7.7.jar" ;;
    0.10.* ) echo "$sbt_launch_repo/sbt/sbt-launch/$version/sbt-launch.jar" ;;
    0.11.[12]) echo "$sbt_launch_repo/sbt/sbt-launch/$version/sbt-launch.jar" ;;
    *) echo "$sbt_launch_repo/sbt/sbt-launch/$version/sbt-launch.jar" ;;
  esac
}
...
declare sbt_launch_repo="http://lang.goodrain.me"
...
main () {
  if [ $PROXY ];then
    export http_proxy=http://203.88.170.137:18888
    export https_proxy=https://203.88.170.137:18888
  fi
  execRunner "$java_cmd" \
  "${extra_jvm_opts[@]}" \
  "${java_args[@]}" \
